		...
		<?php $teacher_dis_admin = $pratima->teacher_info_display_admin();
		$t_sn = 1;
		...
		$subject_info_admins = $pratima->subject_info();
				$sub_sn = 1;
	...
	if(isset($_POST['update_admin_info'])){
		...
		$checkmatch = $pratima->meadmin_check($current_username,$current_password);
		if($checkmatch){
			$success = $pratima->meadmin_updatepassword($new_password,$new_username);
			if($success){
	...
	?>
		...
		<label><?php $count_student_count=  $pratima->student_count(); 
		...
		<div class="stats-right">
		<label><?php 
		$count_teacher_size=  $pratima->teacher_info_display_admin(); 
